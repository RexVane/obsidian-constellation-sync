...
This repository intentionally contains no user tokens, refresh tokens, client secrets, local vault data or `.env.local` files. The checked-in build is produced with placeholder GitHub App settings. To run a configured build, create `.env.local` from `.env.example` and provide the public Client ID and installation URL for a GitHub App that you control. A GitHub OAuth Client ID is public metadata; never put the App client secret in the plugin or in Git history.
...
The public repository is source-first. Each user or release maintainer should build with their own public GitHub App Client ID; no client secret is required or accepted by the plugin.
...
本公开仓库不包含用户令牌、刷新令牌、GitHub App Client Secret、本地笔记、`.env.local` 或 Obsidian 配置。仓库中的构建产物使用占位 GitHub App 配置；运行前请从 `.env.example` 创建 `.env.local`，填写你自己 GitHub App 的公开 Client ID 和安装地址。Client ID 属于公开元数据，但 App Client Secret 绝不能放入插件或 Git 历史。
...
本项目采用源码优先的公开发布方式。每位用户或发布维护者都应使用自己管理的公开 GitHub App Client ID 构建；插件不需要、也不会接受 Client Secret。
