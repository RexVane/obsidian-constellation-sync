...
5. **Repository access（仓库范围）**：选择 **Only select repositories**，只勾选专用于同步的仓库。强烈建议使用私有仓库。
...
## 备选方式：经典令牌（Classic token，权限范围更广）

为兼容已有使用方式，插件仍支持经典令牌，但上面的细粒度令牌是更安全的默认选择。

1. 打开 GitHub 的经典令牌表单：<https://github.com/settings/tokens/new?description=Constellation%20Sync&scopes=repo>。链接会预填名称和 `repo` 权限。
...
| 创建方式 | 插件直接打开 | 手动打开兼容链接 |
| 适合场景 | 长期使用、注重最小权限 | 已有经典令牌工作流 |
...
- 强烈建议使用私有仓库。插件允许公开仓库，但其中的所有同步文件都会对所有人可见。
